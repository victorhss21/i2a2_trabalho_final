...
class ResultExporter:
    ...
    def export(self, catalog: Dict[str, Any]):
        """Exporta para JSON e/ou Excel"""
        os.makedirs(self.config.results_dir, exist_ok=True)
        
        if self.config.export_json:
            self._export_json(catalog)
        
        if self.config.export_excel:
            self._export_excel(catalog)
    
    def _export_json(self, catalog: Dict[str, Any]):
        """Exporta para JSON"""
    ...
    def _export_excel(self, catalog: Dict[str, Any]):
